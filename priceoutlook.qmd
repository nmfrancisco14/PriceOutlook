...
#   },
#   error = function(e) {
#     invisible(NULL) # walang print o message
#   }
# )
...
data_edited[["wholesale_prices"]] <- 
  ...
  mutate_at(vars(data.wmr,data.rmr),as.numeric) |> 
  select(-c(data.monthyr,data.created_at,data.updated_at)) |> 
  relocate(date,.before=1) |> 
  pivot_longer(c(data.wmr,data.rmr), names_to = 'type',values_to = 'price') |> 
  arrange(date, type)
...
data_edited[["farmgate_prices"]] <- 
  ...
  mutate(date = mdy(data.monthyr),
         farmgate = as.numeric(data.farmgate),
         farmgate_rice = farmgate/0.63) |> 
  select(-c(data.farmgate,data.monthyr,data.created_at,data.updated_at)) |> 
  relocate(date,.before=1) |> 
  arrange(date)
...
data_edited[["ipp"]] <- 
  data$fao_fob_prices |>
  left_join(data$exrates |> select(data.monthyr,data.exrates), by="data.monthyr") |> 
  mutate(date =mdy(data.monthyr),
         price = as.numeric(data.value),
         exrates = as.numeric(data.exrates)) |> 
  select(-c(data.monthyr,data.value,data.created_at,data.updated_at)) |> 
  relocate(date,.before=1) |>
...
pricedata_comb <-
  data_edited$ipp_ave |> 
  rename(ipp=price) |> 
  left_join(data_edited$retail_prices |> filter(type=="data.rmr") |> select(date,rprice=price)) |> 
  
  left_join(data_edited$wholesale_prices |> filter(type=="data.rmr") |> select(date,wprice=price)) |> 
  
  left_join(data_edited$farmgate_prices |> select(date,farmgate)) |> 
  left_join(data_edited$ipp |> filter(data.type=="Vietnam 25%") |> select(date,viet_price=price)) |> 
  left_join(data_edited$ipp |> filter(data.type=="Thai 25%") |> select(date,thai_price=price)) |> 
  left_join(data_edited$ipp |> filter(data.type=="Pakistan 25%") |> select(date,pak_price=price)) |> 
  left_join(data$imports |> 
              mutate(date = mdy(monthyr)) |> 
              select(date,imports = imports_nrp) |> 
              add_row(date = mdy("07-01-2025"), imports ="305277.4")) |> 
  left_join(data$totalstocks |> 
              filter(locCode==999) |> 
              mutate(date = ym(paste0(year,"-",month))) |> 
              select(date,household:total)
            ) |> 
  mutate_at(vars(imports:total), as.numeric)
...
base_plot <- 
  ...
  add_lines(data =data_edited[["ipp"]] |> 
            ...
            y=~ipp,
            legendgroup = "global prices",
            name = ~paste(data.type," IPP"),
            hovertemplate = paste0("%{fullData.name}: ",
                                   "₱ %{y:.2f}<extra></extra>"), 
            line = list(color = "#FF3F3F", dash = "dash")) |>
  add_lines(data =data_edited[["ipp"]] |> 
              filter(data.type == "Thai 25%"),
            x=~date,
            y=~ipp,
            legendgroup = "global prices",
            name = ~paste(data.type," IPP"),
...
base_plot |> 
  add_lines(data = pricedata_yoy,
              # data_external[["stocks"]] |> 
              # filter(date>=ymd("2016-01-01"),
              #        Stock == "Commercial"),
            x=~date,
            y=~commercial,
            name = "commercial Stocks",
            # color = ~Stock,
            # customdata = ~Stock,
            # fillcolor = ~Stock,
            line = list(color = "#fbbb18", width = 1),
...
plot_ly(ccf_dfs$commercial$rprice,
        ...
        y = ~Correlation,
        type = "bar",
        name = "CCF: Commercial Stocks on Retail",
        width = 1000,
        height = 400) |>
  layout(xaxis = list(title = "Lag (Months)"),
         yaxis = list(title = "Cross-Correlation"),
...
### Price change using elasticities and market equilibrium {.tiny-text}
...
### Estimated Effect of 2-month Import Ban

::: {.extra-tiny-text}
The effect of a **2-month import ban** on rice, based on the elasticity-incidence computation, is projected to cause an **increase of 5% to 15%** in both **farmgate** and **rice retail** prices, **assuming total transmission** of retail price changes to farmgate prices.
...
data.frame(
  ...
  price_date = c("July 2025","June 2025"),
  price = c(41.31,16.99),
  ...
  rename(Form = rice_form, 
         Date = price_date,
         Price = price,
         `Supply shock` = supplyshock,
         Source = source,
         `Demand elasticity` = demand_elas,
         `Change in price` = pricechange,
         `New price` = newprice,
         `Price gap` = pricegap) |> 
  mutate(across(where(is.numeric), ~ round(.,3)))


  kable(newprice, format = "markdown", caption = "Table. Estimated Price Impact of a 2-Month Rice Import Ban") 
