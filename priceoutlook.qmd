...
granger_summary <- tribble(
  ...
  "Global (IPP) → Local Retail",               2,    round(granger1$`F`[2], 3),  round(granger1$`Pr(>F)`[2], 4), ifelse(granger1$`Pr(>F)`[2] < 0.05, "Yes", "No"),
  "Global (IPP) → Local Farmgate",            2,    round(granger2$`F`[2], 3),  round(granger2$`Pr(>F)`[2], 4), ifelse(granger2$`Pr(>F)`[2] < 0.05, "Yes", "No"),
  "Local Retail → Local Farmgate",            6,    round(granger3$`F`[2], 3),  round(granger3$`Pr(>F)`[2], 4), ifelse(granger3$`Pr(>F)`[2] < 0.05, "Yes", "No")
)
...
digraph {
  ...
  B -> C [label="Lag 0–6 months", color="steelblue"]
}
...
## Other macro-economic factors affecting domestic prices {.tiny-text auto-animate="true"}

:::: {.r-stack}

::: {.fragment fragment-index=1}

### Import Volume  ↔  Local Retail price

The CCF indicates a two-way relationship between imports and retail prices. Notably, import volumes show moderate positive correlations at lags 0 to +2, suggesting that changes in imports tend to coincide with, or lead to, shifts in local retail prices within the same month or within the next 1–2 months.
...
Since we have established that retail prices influence and lead farmgate prices, a similar pattern is expected for farmgate prices—showing an increasing trend in year-on-year (YoY) percentage decline as retail prices continue to fall. 
...
- **Stock Adjustments**  
  ...
  – **Government/NFA Stocks** have been rebuilt at lower cost, stabilizing overall reserve levels.
